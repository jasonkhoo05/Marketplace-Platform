...
// IMPORTING YOUR REUSABLE LAYOUT AND STATE PRIMITIVES
...
export default function SellerOrdersPage() {
    ...
    return (
        <main className="min-h-screen bg-slate-50 pl-60">
            ...
            <div className="p-6">
                {/* RESTORED: Reusable Card framework wraps around your standard table elements */}
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {orders.map((order) => (
                                    ...
                                    >
                                        ...
                                        {/* REUSED: Clean Badge selection state drop targets */}
                                        <td className="px-6 py-4 relative overflow-visible">
                                            <button
                                                onClick={() => setOpenDropdownId(openDropdownId === order.id ? null : order.id)}
                                                className="inline-flex items-center gap-2 rounded-full transition-all focus:outline-none"
                                            >
                                                <Badge variant={order.status.toLowerCase() as any}>
                                                    {order.status}
                                                    <FiChevronDown className={`ml-1 transition-transform ${openDropdownId === order.id ? "rotate-180" : ""}`} />
                                                </Badge>
                                            </button>
